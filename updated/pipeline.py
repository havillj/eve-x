...
def blastScaffolds(dirName, force = False):
    """BLAST scaffolds against viral and aegypti genomes and combine to
        identify putative viral insertions.

       Parameter:
           dirName:   absolute path of parent of directory containing BLAST scaffolds
...
def readCSV(csvFilename):
    ...
    try:
        ...
        return None
    hits = {}
    accsFound = {}		#dictionary of dictionaries
    for line in csv:
        line = line.rstrip()
        row = line.split(',')
...
def reverseComplement(dna):
    """Return the reverse complement of dna.
       ...
       Parameters:
           dna:      a string representation of dna

       Return value: the complement of the dna
    """
...
def getSeqRecord(fileNameFASTA):
    """Reads a FASTA file and returns the reference sequence record

       Parameters:
          fileNameFASTA:      path of a FASTA file containing reference sequence

       Return value: reference sequence record
    """
    try:
...
def writeSummaryTable(fileName, viralHits, viralHits0, viralHits1, viralHits2, allFamilies, allViruses):
    """Writes a tab-delimited summary table of results

       Parameters:
           fileName:            a file containing a tab-delimited table summary of viral hits
           viralHits:           a dictionary of all viral hits
           viralHits0:          a dictionary of viral hits with Aedes aegypti overlap
           viralHits1:          a dictionary of viral hits with non-mathcing flanking regions
           viralHits2:          a dictionary of vrial hits with matching flanking regions
           allFamilies:         a list of all families of viruses
           allViruses:          a list of all viruses
...
def getGenBank(accessionID):
	"""Retrieves and writes reference genome to a file

       Parameters:
            accessionID: an accession ID

       Return value: Boolean indicating whether or not data was accessed succesfully
    """
...
def drawVirus(acc, family, hits, allSpecimens, separatePops, isFamily, showAaegL5_hits, showPlot, small):
    """Draw diagrams of viruses

       Parameters:
           acc:             an accession number
           family:          a family of viruses
           hits:            a dictionary of viral hits
           allSpecimens:    a list of all Aedes aegypti specimens
           separatePops:    boolean for whether or not to make separate plots by population
           isFamily:        boolean for whether the virus is in family or not
           showAaegL5_hits: boolean for whether or not to show AaegL5_hits
           showPlots:       boolean for whether or not to show plots
           small:           boolean for whether or not to make plots small
...
def getHitsForDiagram():
    """Retrieves all virus hits and specimens for diagrams

       Parameters: None

       Return value:
            allVirusHits:   a dictionary of all virus hits
            allSpecimens:   a list of Aedes aegypti specimens
    """
...
def drawFamily(families, famACCs, separatePops, showAaegL5_hits, showPlot, small):
    """Draws a diagram for each family of viruses.

       Parameters:
            families:       a list of virus families
            famACCs:        a list of accession numbers
            separatePops:    boolean for whether or not to make separate plots
            showAaegL5_hits: boolean for whether or not to show AaegL5_hits
            showPlots:       boolean for whether or not to show plots
            small:           boolean for whether or not to make plots small
...
def doIt(dirName, filenameBAM):
    """Runs through the entire pipeline for one specimen

       Parameters:
            dirName:        absolute path of directory containing input BAM files
            filenameBAM:    absolute path of input BAM file
...
def doAll():
    """Runs through the entire pipeline for all specimens
